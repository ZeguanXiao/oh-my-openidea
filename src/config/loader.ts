...
const PROMPTS_DIR_NAME = 'oh-my-openidea';
...
function loadConfigFromPath(configPath: string): PluginConfig | null {
  try {
    ...
    if (!result.success) {
      console.warn(`[oh-my-openidea] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      console.warn(
        `[oh-my-openidea] Error reading config from ${configPath}:`,
        error.message,
      );
    }
    return null;
 ...
 *
 * @param basePath - Base path without extension (e.g., /path/to/oh-my-openidea)
 ...
 * 1. User config: ~/.config/opencode/oh-my-openidea.jsonc or .json (or $XDG_CONFIG_HOME)
 * 2. Project config: <directory>/.opencode/oh-my-openidea.jsonc or .json
 *
...
export function loadPluginConfig(directory: string): PluginConfig {
  ...
  const envPreset = process.env.OH_MY_OPENIDEA_PRESET;
  if (envPreset) {
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[oh-my-openidea] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
  ...
  const readFirstPrompt = (
    ...
    for (const dir of promptSearchDirs) {
      ...
      try {
        ...
        console.warn(
          `[oh-my-openidea] ${errorPrefix} ${promptPath}:`,
