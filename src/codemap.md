...
- Research tools (`tools/idea-store`) provide idea persistence behind the OpenCode `tool` interface. Paper search and reading are now provided via MCPs (AlphaXiv MCP, Google Scholar MCP).
...
- Plugin registration: `index.ts` registers agents, the tool map (background/task, `idea_store`), MCP definitions (AlphaXiv, Google Scholar, Exa websearch, Zotero), and hooks; configuration hook merges those values back into the OpenCode config (default agent, permission rules, and MCP access policies).
...
- Integrates with external services via MCP: AlphaXiv MCP (arXiv paper search and reading), Google Scholar MCP (academic search via `scholarly` Python library, no API key required), Exa web search MCP, Zotero personal library MCP.
