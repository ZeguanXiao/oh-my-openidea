...
- Research tools (`tools/idea-store`) provide idea persistence behind the OpenCode `tool` interface. Paper search and reading are provided via MCPs (AlphaXiv MCP, Exa websearch MCP).
...
- Plugin registration: `index.ts` registers agents, the tool map (background/task, `idea_store`), MCP definitions (AlphaXiv, Exa websearch, Zotero), and hooks; configuration hook merges those values back into the OpenCode config (default agent, permission rules, and MCP access policies).
...
- Integrates with external services via MCP: AlphaXiv MCP (arXiv paper search and reading), Exa web search MCP, Zotero personal library MCP.
