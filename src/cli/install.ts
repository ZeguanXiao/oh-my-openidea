...
function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(
    `${BOLD}oh-my-openidea ${isUpdate ? 'Update' : 'Install'}${RESET}`,
  );
...
async function runInteractiveMode(
  ...
  try {
    ...
    console.log();
    console.log(`${BOLD}oh-my-openidea Setup${RESET}`);
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  if (!modelsOnly) {
    printStep(step++, totalSteps, 'Adding oh-my-openidea plugin...');
    if (resolvedConfig.dryRun) {
  ...
  printStep(step++, totalSteps, 'Writing oh-my-openidea configuration...');
  if (resolvedConfig.dryRun) {
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  if (!args.tui) {
    ...
    if (errors.length > 0) {
      ...
      console.log();
      printInfo(
        'Usage: bunx oh-my-openidea install --no-tui --kimi=<yes|no> --openai=<yes|no> --anthropic=<yes|no> --copilot=<yes|no> --zai-plan=<yes|no> --antigravity=<yes|no> --chutes=<yes|no> --balanced-spend=<yes|no> --tmux=<yes|no>',
      );
      console.log();
      return 1;
    }
