...
The `src/cli/` directory provides the command-line interface for installing and configuring **oh-my-openidea**, an OpenCode plugin. It handles:
...
3. **Provider Priority** (`providers.ts`):
   ...
   - Hybrid mode: Kimi for orchestrator/synthesizer, OpenAI for critic
...
```
User runs: bunx oh-my-openidea install
         │
         ▼
┌─────────────────────────────────────────┐
│ index.ts: parseArgs()                   │
...
└─────────────────┬───────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────┐
│ For each agent (orchestrator, surveyor, │
│ synthesizer, critic, architect, writer):│
│                                         │
...
| `oh-my-openidea.json` | `~/.config/opencode/` | Plugin-specific config |

### Consumers

1. **End Users**: Via `bunx oh-my-openidea install`
