...
No API key required.`,
  ...
  args: {
    ...
    max_chars: z
      ...
      .optional()
      .describe(
        'Maximum characters to return from the full text (default: 20000). Use a smaller value to focus on the beginning of the paper.',
      ),
  },
  ...
  async execute(args) {
    ...
    const maxChars =
      typeof args.max_chars === 'number' ? args.max_chars : 20_000;
    ...
    try {
      ...
      return (
        ...
        (truncated
          ? `(Showing first ${maxChars.toLocaleString()} of ${text.length.toLocaleString()} characters)\n`
          : '') +
        `\n${output}`
      );
    } catch (err) {
